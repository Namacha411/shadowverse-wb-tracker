<script lang="ts">
	...
	import { getDecks, createRecord } from '$lib/db';
	...
	let redirectMsg = $state('');

	onMount(async () => {
		decks = await getDecks();
		if (decks.length === 0) {
			redirectMsg = 'デッキが未登録です。先にデッキを登録してください。';
			setTimeout(() => goto('/decks/new'), 1500);
			return;
		}
		deckHash = decks[0].hash;
	});
...
{#if redirectMsg}
	<p class="redirect-msg">{redirectMsg}</p>
{:else}
	...
	<form onsubmit={handleSubmit}>
		...
		<div class="actions">
			...
			<a href="/records" class="btn-cancel">キャンセル</a>
		</div>
	</form>
{/if}
